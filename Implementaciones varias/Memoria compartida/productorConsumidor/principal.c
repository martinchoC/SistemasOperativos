...
int main(){
	...
	if (mem == -1){
		printf("No puedo crear correctamente memoria compartida \n");
		exit (0);
	}
	//borro la memoria
	...
	sem_init(&(mcompartida->productos),1,20);
	sem_init(&(mcompartida->producir),1,0);

	mcompartida->cantProductos = 20;

	//creo todos los procesos hijo (1 productor y 1 consumidor).
	...
	while (i<PRODUCTORES)
	{
		int a=fork();
		if (a==0){
			 int r1 = execlp("./productor","productor",NULL);
			if (r1 == -1) 
				return (1);
		} //si falla termina
		else //soy el padre
	...
	i=0;
	while (i<CONSUMIDORES)
	{
		int b=fork();
		if (b==0){
			int q = execlp("./consumidor","consumidor",NULL);
			if (q == -1) 
				return (1);
		} //si falla termina
		else //soy el padre
