...
#define COLA_COMIDA_CARNE 1239
#define COLA_COMIDA_VEGE 1240
...
void crearColas() {
    ...
    int colaComidaCarne = msgget((key_t)COLA_COMIDA_CARNE, 0666 | IPC_CREAT);
    if (colaComidaCarne < 0)
    {
        perror("cola");
        exit(EXIT_FAILURE);
    }

    int colaComidaVege = msgget((key_t)COLA_COMIDA_VEGE, 0666 | IPC_CREAT);
    if (colaComidaVege < 0)
    {
        perror("cola");
        exit(EXIT_FAILURE);
...
void* cliente(void* nombre) {
    ...
    int colaPedidos = msgget((key_t)COLA_PEDIDOS, 0666 | IPC_CREAT);
    int colaComidaLista = msgget((key_t)COLA_COMIDA_LISTA, 0666 | IPC_CREAT);
    tMensaje msjMesa, msjPedido, msjCamarero, msjComidaLista;
    while(1) {
        srand (time(NULL));
        ...
        {
            msjPedido.type = TIPO_VEGE;
            msjCamarero.type = TIPO_VEGE;
            printf("El cliente %d pide comida vegetariana.\n", nroCliente+1);
        }        
        sleep(1);
        msgsnd(colaPedidos, &msjPedido, SIZE_MSG, IPC_NOWAIT);
        msgsnd(colaCamarero, &msjCamarero, SIZE_MSG, IPC_NOWAIT);
        
        //sleep(1);
        msgrcv(colaComidaLista, &msjComidaLista, SIZE_MSG, 0, 0);
        if (msjComidaLista.type == TIPO_CARNE)
        {
            printf("Comida con carne servida al al cliente %d.\n",nroCliente+1);
        }
        else if (msjComidaLista.type == TIPO_VEGE)
        {
            printf("Comida vegetariana servida al al cliente %d.\n",nroCliente+1);
        }
        sleep(1);
...
void* camarero() {
    ...
    while (1)
    {
        ...
        {
            msgrcv(colaComidaCarne, &msjComida, SIZE_MSG, TIPO_CARNE, 0);
            printf("El camarero busca una comida con carne\n");
        }
        else
        {
            msgrcv(colaComidaVege, &msjComida, SIZE_MSG, TIPO_VEGE, 0);
...
void* cocinero(void* nombre) {
    ...
    while (1)
    {
        msgrcv(colaPedidos, &msjComida, SIZE_MSG, 0, 0);
        msgrcv(colaDisponible, &msjDisponible, SIZE_MSG, 0, 0);
        if (msjComida.type == TIPO_CARNE)
        {
        ...
        if (msjComida.type == TIPO_CARNE)
        {
            ...
            msgsnd(colaComidaCarne, &msjComida,SIZE_MSG,IPC_NOWAIT);
        }
        else if (msjComida.type == TIPO_VEGE)
        {
            printf("El cocinero %d terminó una comida vegetariana y la agregó a la cola.\n",nroCocinero+1);
            msgsnd(colaComidaVege, &msjComida,SIZE_MSG,IPC_NOWAIT);
        }
        sleep(1);
    }
    pthread_exit(0);
